...
class BasesfApplyActions extends sfActions
{
  ...
  {
    // The user most likely decided to log in instead when they
    // reached the Create an Account page. This is common with
    // Facebook signin, so cope with it in a reasonable way
    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        $this->form->save();
        $this->postCreate($this->form->getObject());
        try
        {
  ...
  /**
   * This is a good override point to drop the user
   * into a 'guests' group or similar. The user already
   * exists in the database at this point, so they
   * have an id. They have NOT necessarily confirmed
   * their email yet, you can check $user->getIsActive()
   * to determine that.
   */
  protected function postCreate($user)
  {
    ...
    else
    {
      ...
      if ($request->isMethod('post'))
      {
        ...
        if ($this->form->isValid())
        {
          ...
          {
            $user = Doctrine_Core::getTable('sfGuardUser')->createQuery('u')->where('u.email_address = ?', $username_or_email)->fetchOne();
            
          }
          else
          {
            $user = Doctrine_Core::getTable('sfGuardUser')->createQuery('u')->where('u.username = ?', $username_or_email)->fetchOne();
          }
  ...
  public function executeConfirm(sfRequest $request)
  {
    ...
    $sfGuardUser = Doctrine_Core::getTable('sfGuardUser')->findOneByValidate($validate);
    if (!$sfGuardUser)
    {
      return 'Invalid';
    }
